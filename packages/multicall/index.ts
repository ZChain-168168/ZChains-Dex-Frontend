...
export const multicallCreateBlockNumber = {
  1: 462418,
  4: 462418,
  5: 462418,
  56: 462418,
...
export function createMulticall<TProvider extends Provider>(
  ...
  defaultActiveChainId: number,
) {
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = defaultActiveChainId) => {
  ...
  const multicallv3 = async ({ calls, chainId = defaultActiveChainId, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
