...
export async function farmV2FetchFarms({
  ...
  const [stableFarmsResults, poolInfos, lpDataResults] = await Promise.all([
    ...
    fetchMasterChefData(farms, chainId, multicallv2, masterChefAddress), // fetch poolInfo
    fetchPublicFarmsData(farms, chainId, multicallv2, masterChefAddress), // balanceOf, totalSupply
  ])
