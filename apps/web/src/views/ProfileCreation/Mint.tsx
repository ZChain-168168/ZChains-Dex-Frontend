...
const Mint: React.FC<React.PropsWithChildren> = () => {
  ...
  useEffect(() => {
    const getStarterNfts = async () => {
      const response = await getNftsFromCollectionApi(getPancakeBunniesAddress())
      if (!response) return
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
