      ...
      {Number.isFinite(lockedCakeBalance) && (
        <Box>
          <Text>
            {isBoostingExpired
              ? t(
                  `Your v${cstValue.tokenName} boosting was expired at the snapshot block. Renew your fixed-term staking position to activate the boost for future voting proposals.`,
                )
              : t(
                  `Voting power is calculated using the staking amount and remaining staking duration of the fixed-term ${cstValue.tokenName} staking position at the block.`,
                )}
          </Text>
          <Text bold m="10px 0">
            {`${t(`${cstValue.tokenName} locked:`)} ${formatNumber(lockedCakeBalance, 0, 2)}`}
          </Text>
  ...
  return (
    <ModalInner mb="0">
      <Text as="p" mb="24px" fontSize="14px" color="textSubtle">
        {t(
          `Your voting power is determined by the amount of ${cstValue.tokenName} you held and the remaining duration on the fixed-term staking position (if you have one) at the block detailed below. ${cstValue.tokenName} held in other places does not contribute to your voting power.`,
        )}
      </Text>
      ...
      {Number.isFinite(cakeVaultBalance) && (
        <Flex alignItems="center" justifyContent="space-between" mb="4px">
          <Text color="textSubtle" fontSize="16px">
            {t(`Flexible ${cstValue.tokenName} Staking`)}
          </Text>
        ...
        <>
          {lockedCakeBalance === 0 ? (
            <Flex alignItems="center" justifyContent="space-between" mb="4px">
              <Flex>
                <Text color="textSubtle" fontSize="16px">
                  {t(`Fixed Term ${cstValue.tokenName} Staking`)}
                </Text>
                ...
                <Flex mt="10px" flexDirection="column" alignItems="flex-end">
                  ...
                  <Flex>
                    ...
                    <Text ml="4px" color={isBoostingExpired ? 'warning' : 'secondary'} fontSize="12px">
                      {isBoostingExpired ? t('Boosting Expired') : t(`Boosted by v${cstValue.tokenName}`)}
                    </Text>
                  </Flex>
                </Flex>
              </FixedTermCardInner>
