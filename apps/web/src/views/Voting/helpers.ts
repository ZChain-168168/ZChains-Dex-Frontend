...
import { ADMINS, PANCAKE_SPACE, SNAPSHOT_VERSION, STRATEGIES, NETWORK } from './config'
...
export const getVotingPower = async (
  ...
  if (blockNumber && (blockNumber >= VOTING_POWER_BLOCK.v0 || blockNumber >= VOTING_POWER_BLOCK.v1)) {
    ...
    const version = blockNumber >= VOTING_POWER_BLOCK.v1 ? 'v1' : 'v0' // edit
    ...
    const [cakeBalance, cakeBnbLpBalance, cakePoolBalance, cakeVaultBalance, poolsBalance, total, ifoPoolBalance] = await getScores(
        PANCAKE_SPACE,
        [
          strategies.cakeBalanceStrategy(version), // edit
