...
import { useAllBlockchain } from 'state/home/fetchAllBlockChain'
import { avalanche, creditChain, isChainSupported } from 'utils/wagmi'
  ...
  return (
    ...
    <Styles.CurrenciesSelectContentStyle>
      {currencyListByChain?.length > 0 ? (
        ...
        <Flex alignItems="center">
          <EmptyIcon width={24} />
          <Text color="#AEAFB0" as="p" ml="16px" fontSize={[12, 14]} fontWeight={500}>
            There Are No Data
          </Text>
        </Flex>
      )}
...
const Bridge = ({ pageSupportedChains }: { pageSupportedChains: number[] }) => {
  ...
  const getToChain = async () => {
    ...
    const supportedToChain = _.intersection(
      [creditChain, avalanche].map((item) => item.network),
      listToBlockchain,
    )
        ...
        <div className="content">
          <div className="form">
            ...
            <Box background="#000000" paddingY="10px" paddingX="10px" borderRadius="15px" mb={2} mt={3}>
                        ...
                        <Styles.RightInputButton>
                          <Box className="wIcon">
                            {formValue.currency ? (
                              ...
                              <HelpIcon color="#AEAFB0" />
                            )}
                          </Box>
                          <Box ml="1px">
                            <ArrowDownIcon style={{ color: 'red' }} fill="#AEAFB0" color="#AEAFB0" />
                          </Box>
                        </Styles.RightInputButton>
                      }
                    >
              ...
              <Text color="#AEAFB0" fontSize={[10, , 12]}>
                {' '}
                Balance: {currencyBalance > 0 ? currencyBalance : '--'}
              </Text>
            </Box>
            {/* To */}
          ...
          {formValue.fromNetwork && formValue.currency && formValue.sendAmount > 0 && (
            <div className="card-info">
              {/* <Flex justifyContent="space-between" mb="8px">
                <Text fontSize={['14px', '', '16px']} color="rgb(240, 185, 11)">
                  System Fee ({formValue.currency?.system_fee || '--'}%):
                </Text>
                <Text fontSize={['14px', '', '16px']} color="#fafafa">
