...
const AccessRiskComponent: React.FC<AccessRiskProps> = ({ token }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      ...
      <Text my="8px">
        {t(
          'It is a tool for indicative purposes only to allow users to check the reference risk level of a CREDIT Chain Smart Contract. Please do your own research - interactions with any CREDIT Chain Smart Contract is at your own risk.',
        )}
      </Text>
      <Flex mt="4px">
      ...
      <Flex justifyContent="flex-end">
        <div ref={targetRef} style={{ userSelect: 'none' }}>
          <Tag variant={data.riskLevel > TOKEN_RISK.MEDIUM ? 'failure' : 'primary'}>
            <Text bold small color="text">
              {hasRiskValue}
            </Text>
            {tooltipVisible && tooltip}
            <Flex>
              <HelpIcon ml="4px" width="16px" height="16px" color="text" />
            </Flex>
          </Tag>
        </div>
      </Flex>
