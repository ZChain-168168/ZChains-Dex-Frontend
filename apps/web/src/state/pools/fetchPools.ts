...
const livePoolsWithEnd = poolsConfig.filter((p) => !p.isFinished)
...
const startEndBlockCalls = livePoolsWithEnd.flatMap((poolConfig) => {
  return [
    {
      address: getAddress(poolConfig.contractAddress, DEFAULT_ACTIVE_CHAIN_ID),
      name: 'startBlock',
    },
    {
      address: getAddress(poolConfig.contractAddress, DEFAULT_ACTIVE_CHAIN_ID),
...
export const fetchPoolsBlockLimits = async () => {
  ...
  const result = livePoolsWithEnd.map((cakePoolConfig, index) => {
...
const poolsBalanceOf = poolsConfig.map((poolConfig) => {
  return {
    ...
    name: 'balanceOf',
    params: [getAddress(poolConfig.contractAddress, DEFAULT_ACTIVE_CHAIN_ID)],
  }
})
...
export const fetchPoolsStakingLimits = async (
  ...
  const validPools = poolsConfig
    .filter((p) => p.stakingToken.symbol !== WNATIVE[chainId].symbol && !p.isFinished)
...
export const fetchPoolsProfileRequirement = async (): Promise<{
  ...
  const poolProfileRequireCalls = livePoolsWithV3
    .map((validPool) => {
      const contractAddress = getAddress(validPool.contractAddress, DEFAULT_ACTIVE_CHAIN_ID)
