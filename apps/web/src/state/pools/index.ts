...
export const fetchCakePoolUserDataAsync = (account: string) => async (dispatch) => {
  const allowanceCall = {
    address: CAKE[DEFAULT_ACTIVE_CHAIN_ID].address,
    name: 'allowance',
...
export const fetchPoolsPublicDataAsync =
  (currentBlockNumber: number, chainId: number) => async (dispatch, getState) => {
    try {
      ...
      const poolsWithDifferentFarmToken = activePriceHelperLpsConfig.length > 0 ? await fetchFarms(priceHelperLpsConfig, chainId) : []
      // console.log('poolsWithDifferentFarmToken', poolsWithDifferentFarmToken)
      const farmsData = getState().farms.data
      // console.log('farmsData', farmsData)
      const bnbBusdFarm = activePriceHelperLpsConfig.length > 0 ? farmsData.find((farm) => farm.token.symbol === 'BUSD' && farm.quoteToken.symbol === WNATIVE[chainId].symbol) : null
      // console.log('bnbBusdFarm', bnbBusdFarm)
      const farmsWithPricesOfDifferentTokenPools = bnbBusdFarm ? getFarmsPrices([bnbBusdFarm, ...poolsWithDifferentFarmToken], chainId) : []
      ...
      const liveData = poolsConfig.map((pool) => {
        ...
        // console.log('pool.tokenPerBlock', parseFloat(pool.tokenPerBlock))
