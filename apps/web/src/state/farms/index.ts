...
async function getNormalFarmsStakeValue(farms, account, chainId) {
  const [userFarmAllowances, userFarmTokenBalances, userStakedBalances, userFarmEarnings] = await Promise.all([
    fetchFarmUserAllowances(account, farms, chainId), // fetch account allowances by masterChefAddress
    fetchFarmUserTokenBalances(account, farms, chainId), //  fetch account balanceOf lpAddress
    fetchFarmUserStakedBalances(account, farms, chainId), // fetch userInfo farm
    fetchFarmUserEarnings(account, farms, chainId), // fetch pendingCake farm
  ])
